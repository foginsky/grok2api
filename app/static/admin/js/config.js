...
const LOCALE_MAP = {
  ...
  "proxy": {
    ...
    "skip_proxy_ssl_verify": { title: "跳过代理 SSL 校验", desc: "启用后将跳过代理连接的 SSL 证书校验，仅建议在自签名证书等特殊环境下使用。" },
  ...
  "retry": {
    ...
    "reset_session_status_codes": { title: "会话重置状态码", desc: "遇到这些 HTTP 状态码时自动重建上游会话，避免异常连接状态持续影响后续请求。" },
